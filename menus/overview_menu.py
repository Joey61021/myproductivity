...
def show():
    ...
    if not date_logged(date_today):
        ...
        widget.button(log_frame, "Log Date", None, widget_size, False, colors.button_green, 'white', lambda: [None, metric_menu.show(), menu.destroy(base)][0]).pack(padx=10, pady=10, side='right')
    ...
    for row in range(0, rows):
        for col in range(0 if row != 0 else utils.convert_date(entries[0][0]).weekday()+1, 7 if row != rows-1 else utils.convert_date(entries[len(entries)-1][0]).weekday()+1):
            if date_logged(curr_date):
                widget.button(button_frame, str(entries[curr_entry][0]).replace('-', '/'), None, widget_size, False, get_color(int(entries[curr_entry][1])), 'white', None).grid(row=row, column=col, padx=1, pady=1)
                curr_entry += 1
            else:

                widget.button(button_frame, "No data", None, widget_size, False, colors.button_null, 'white', None).grid(row=row, column=col, padx=1, pady=1)
